...
case "$TEST_SUITE" in
    ...
    "lms-unit")
        ...
        case "$SHARD" in
            "all")
                paver test_system -s lms $PAVER_ARGS 2> lms-tests.log
                ;;
            [1-3])
                paver test_system -s lms --attr="shard=$SHARD" $PAVER_ARGS 2> lms-tests.$SHARD.log
                ;;
            4|"noshard")
                paver test_system -s lms --attr='!shard' $PAVER_ARGS 2> lms-tests.REST.log
                ;;
            *)
    ...
    "cms-unit")
        paver test_system -s cms --with-flaky --cov-args="-p" -v --with-xunitmp 2> cms-tests.log
        ;;

    "commonlib-unit")
        paver test_lib --with-flaky --cov-args="-p" -v --with-xunit 2> common-tests.log
        ;;
